...
.gruntwork/
  repository.hcl
  environment-__RB_ACCOUNT__.hcl
.github/workflows/
  pipelines.yml
...
| `.gruntwork/environment-__RB_ACCOUNT__.hcl` | Which account this environment deploys into, and the roles to assume |
...
| `__RB_ACCOUNT__/account.hcl` | Account-level values: account id, state bucket |
| `__RB_ACCOUNT__/_global/region.hcl` | Region-level values for global resources |
| `__RB_ACCOUNT__/_global/bootstrap/` | The bootstrap stack (see below) |
...
**The bootstrap stack** (`__RB_ACCOUNT__/_global/bootstrap/terragrunt.stack.hcl`) provisions what
...
cd __RB_ACCOUNT__/_global/bootstrap
...
sed -i.bak \
  ...
  -e "s|__RB_ACCOUNT__|${RB_AccountName}|g" \
