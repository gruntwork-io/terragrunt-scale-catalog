...
log_info "Scaffold generated (${RB_out_generate_bootstrap_generated}). Applying the bootstrap stack..."
...
log_info "Applying in ${RB_SubscriptionName}/bootstrap (creates state storage + plan/apply Entra ID apps)..."
terragrunt run --all --non-interactive --provider-cache --no-stack-generate apply

echo "applied=true" >>"$RUNBOOK_OUTPUT"
log_info "Bootstrap apply complete for subscription ${RB_out_derive_azure_azure_subscription_id}."
