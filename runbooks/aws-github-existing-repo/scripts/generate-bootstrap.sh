#!/bin/bash
# Render the aws/github/account boilerplate template into the cloned repository, using values
# collected from the form and auto-derived from GitHub + AWS. Runs non-interactively.
set -euo pipefail
...
log_info "Rendering aws/github/account template (ref ${CATALOG_REF})..."
"${BOILERPLATE_BIN:-boilerplate}" \
  --template-url "github.com/gruntwork-io/terragrunt-scale-catalog//templates/boilerplate/aws/github/account?ref=${CATALOG_REF}" \
  --output-folder . \
  --var-file vars.yml \
  --non-interactive
