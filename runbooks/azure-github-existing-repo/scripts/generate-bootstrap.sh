#!/bin/bash
# Render the azure/github/subscription boilerplate template into the cloned repository, using values
# collected from the form and auto-derived from GitHub + Azure. Runs non-interactively.
set -euo pipefail
...
log_info "Generated {{ .inputs.SubscriptionName }}/bootstrap/, {{ .inputs.SubscriptionName }}/sub.hcl, and .gruntwork/environment-{{ .inputs.SubscriptionName }}.hcl"
