...
into a GitHub `infrastructure-live` repository and bootstraps one **GCP** project.
...
- **no `root.hcl`** — renders the full repository layout (root config, `.mise.toml`, CI workflows,
  and the first project's bootstrap stack);
...
```bash
runbooks open ./runbooks/gcp-github
```

Or directly from the catalog:

```bash
runbooks open https://github.com/gruntwork-io/terragrunt-scale-catalog/tree/main/runbooks/gcp-github
```
