...
GCPProjectNumber: "${RB_out_read_details_gcp_project_number}"
...
if [ ! -f .mise.toml ] && [ ! -f mise.toml ] && [ ! -f .config/mise/config.toml ]; then
  ...
  mise use "terragrunt@${TERRAGRUNT_VERSION}" "$IAC_PIN" "gcloud@565.0.0"
fi
...
| Path | What it is |
| --- | --- |
| `root.hcl` | Root Terragrunt config: GCS remote state, the google provider, shared inputs |
| `.mise.toml` | Pinned tool versions (Terragrunt, __RB_TOOL__, gcloud) |
| `.gruntwork/repository.hcl` | Repository-wide Pipelines settings: deploy branch, `tf_binary` |
| `.gruntwork/environment-<project>.hcl` | Which GCP project this environment deploys into, and the service accounts to impersonate |
...
| `<project>/README.md` | What this project's bootstrap provisioned, with its identifiers |
| `<project>/pipelines-bootstrap.svg` | The same, drawn with this project's real values |
| `<project>/project.hcl` | Project-level values: project id, project number, state bucket |
| `<project>/bootstrap/` | The bootstrap stack (see below) |

Each additional project is another top-level directory with the same shape, plus its own
`.gruntwork/environment-<project>.hcl`.

## How the pieces fit

**`root.hcl`** is included by every unit. It configures the GCS backend — one state object per unit,
keyed by its path — and the `google` provider. Change it only when something applies to the whole
repository.

**`.gruntwork/repository.hcl`** is Pipelines' repository-wide configuration: the deploy branch, and
`tf_binary` (this repository uses **__RB_TOOL__**). `.gruntwork/environment-<project>.hcl` maps an
environment to the GCP project it deploys into and the service accounts Pipelines impersonates for
plan and apply.

**The bootstrap stack** (`<project>/bootstrap/terragrunt.stack.hcl`) provisions what Pipelines
needs before it can deploy anything into project `__RB_PROJECT_ID__`:

- a Workload Identity Pool and OIDC provider trusting GitHub Actions,
- `__RB_PREFIX__-plan` — a read-only service account, impersonated on pull requests,
- `__RB_PREFIX__-apply` — impersonated on merges to the deploy branch,
- the GCS bucket holding this project's state.

![Example bootstrap topology](pipelines-bootstrap-example.svg)

*Example labels. Every project bootstrapped into this repository also gets its own copy of this
diagram, drawn with that project's real identifiers, at `<project>/pipelines-bootstrap.svg`.*

Workload Identity Federation means no service account keys: GitHub's OIDC token is exchanged for a
short-lived credential at run time.

It is applied once, when the project is onboarded. After that it is ordinary configuration: changes
...
Re-run the Gruntwork Runbook that created this repository and give it a new project name. It adds a
new top-level directory and a new `.gruntwork/environment-<project>.hcl`, and leaves everything here
untouched.
...
sed -i.bak \
  ...
  -e "s|__RB_PROJECT_ID__|${RB_GCPProjectID}|g" \
...
# The illustration for the README's bootstrap section. Unlike the per-project diagrams this one is
...
<svg xmlns="http://www.w3.org/2000/svg" width="760" height="580" viewBox="0 0 760 580" font-family="Helvetica, Arial, sans-serif">
  ...
  <text x="380" y="54" text-anchor="middle" font-size="12" fill="#666666">what the stack provisions in each GCP project</text>
  ...
  <text x="380" y="223" text-anchor="middle" font-size="11" fill="#444444">pool: &lt;prefix&gt;-pool</text>
  <text x="380" y="239" text-anchor="middle" font-size="11" fill="#444444">provider: &lt;prefix&gt;-provider</text>
  <text x="380" y="255" text-anchor="middle" font-size="10" fill="#666666">no service account keys</text>
  ...
  <text x="195" y="362" text-anchor="middle" font-size="10" fill="#444444">&lt;prefix&gt;-plan</text>
  ...
  <text x="565" y="362" text-anchor="middle" font-size="10" fill="#444444">&lt;prefix&gt;-apply</text>
  <text x="565" y="380" text-anchor="middle" font-size="10" fill="#666666">impersonated on merges to the deploy branch</text>
  ...
  <text x="380" y="507" text-anchor="middle" font-size="11" fill="#444444">gs://&lt;state-bucket&gt;</text>
  <text x="380" y="523" text-anchor="middle" font-size="10" fill="#666666">one state file per unit</text>

  <text x="380" y="558" text-anchor="middle" font-size="10" fill="#999999">Example. Each project has its own diagram at &lt;project&gt;/pipelines-bootstrap.svg</text>
