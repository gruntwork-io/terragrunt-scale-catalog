...
.gruntwork/
  repository.hcl
  environment-__RB_PROJECT__.hcl
.github/workflows/
  pipelines.yml
...
| `.gruntwork/environment-__RB_PROJECT__.hcl` | Which GCP project this environment deploys into, and the service accounts to impersonate |
...
| `__RB_PROJECT__/project.hcl` | Project-level values: project id, project number, state bucket |
| `__RB_PROJECT__/bootstrap/` | The bootstrap stack (see below) |
...
**The bootstrap stack** (`__RB_PROJECT__/bootstrap/terragrunt.stack.hcl`) provisions what Pipelines
...
new top-level directory and a new `.gruntwork/environment-<name>.hcl`, and leaves everything here
untouched.
...
sed -i.bak \
  ...
  -e "s|__RB_PROJECT__|${RB_ProjectName}|g" \
