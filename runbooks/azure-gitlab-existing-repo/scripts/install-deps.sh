#!/bin/bash
# Install the tool dependencies (Terragrunt, OpenTofu, the Azure CLI, and Python) pinned in the
...
if [ ! -f .mise.toml ] && [ ! -f mise.toml ] && [ ! -f .config/mise/config.toml ]; then
  ...
  mise use "terragrunt@{{ .inputs.TerragruntVersion }}" "opentofu@{{ .inputs.OpenTofuVersion }}" "azure-cli@2.84.0" "python@3.14.3"
fi
