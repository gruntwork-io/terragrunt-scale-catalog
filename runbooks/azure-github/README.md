...
into a GitHub `infrastructure-live` repository and bootstraps one **Azure** subscription.
...
- **no `root.hcl`** — renders the full repository layout (root config, `.mise.toml`, CI workflows,
  and the first subscription's bootstrap stack);
...
```bash
runbooks open ./runbooks/azure-github
```

Or directly from the catalog:

```bash
runbooks open https://github.com/gruntwork-io/terragrunt-scale-catalog/tree/main/runbooks/azure-github
```
