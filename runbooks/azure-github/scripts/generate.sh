...
#
#   scaffold     - azure/github/infrastructure-live. Renders the whole repository: root.hcl,
#                  .mise.toml, .gitignore, .gruntwork/, the workflows, and the first stack.
#   subscription - azure/github/subscription. Adds only the new subscription's directory and
#                  bootstrap stack to a repository that already has root.hcl.
#
...
EOF
  [ -n "$RB_Issuer" ] && echo "Issuer: \"${RB_Issuer}\"" >> vars.yml
fi
